...
""" Particle filter for benchmark SSM
"""

@njit
def SimpleBenchmarkSSM_bootstrap_pf(temp, n_particles, ys, us, theta, alternative_tempering=False):
    ...
    for t in range(1,T):
        ...
        for i in range(n_particles):
            ...
            xs[i,t] = np.random.normal(
                                    loc=SimpleBenchmarkSSM_transition_mean(
                                        xs[aas[i,t-1],t-1],
                                        theta=theta,
                                        u=us[t-1]
                                        ),
