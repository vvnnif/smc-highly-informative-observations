...
non-linear model in Svensson et al. (2017)
"""

@njit
def Svensson2017NonLinearSSM_log_pos(temp, thetas, ys, xss, aass, alternative_tempering=False):
...
def bisect(prev_log_pos, 
    ...
    nxt_log_pos_lo = Svensson2017NonLinearSSM_log_pos(lo, thetas, ys, xss, aass, alternative_tempering)
    ...
    nxt_log_pos_hi = Svensson2017NonLinearSSM_log_pos(hi, thetas, ys, xss, aass, alternative_tempering)
    ...
    mid = (lo + hi) / 2
    nxt_log_pos_mid = Svensson2017NonLinearSSM_log_pos(mid, thetas, ys, xss, aass, alternative_tempering)
...
@njit(parallel=True)
def Svensson2017NonLinearSSM_parallel_mutate(thetas, 
                          xss, 
                          aass, 
                          log_liks, 
                          temp, 
...
def Svensson2017NonLinearSSM_smc(ys, 
    ...
    theta = Svensson2017NonLinearSSM_sample_prior() # Starting parameter from prior
    ...
    while temp > min_temp:
        ...
        log_pos = Svensson2017NonLinearSSM_log_pos(temp=temp,
                        thetas=thetas,
                        ys=ys,
                        xss=xss,
                        aass=aass)
        ...
        mean_accept_rate = Svensson2017NonLinearSSM_parallel_mutate(
                                            thetas=thetas, 
                                            xss=xss, 
                                            aass=aass, 
                                            log_liks=log_liks, 
